...
const BarangKeluarScreen = () => {
  ...
  const saveOutgoingItems = () => {
    ...
    if (jumlahKeluar > parseInt(barang.jumlah, 10)) {
    ...
    const updatedData = allData
      .filter(item => {
        if (item.kode === barang.kode) {
          const sisa = parseInt(item.jumlah, 10) - jumlahKeluar;
          return sisa > 0; // kalau sisa > 0, tetap simpan
        }
        return true; // item lain tetap simpan
      })
      .map(item => {
        if (item.kode === barang.kode) {
          return {
            ...item,
            jumlah: (parseInt(item.jumlah, 10) - jumlahKeluar).toString(),
          };
        }
        return item;
      });
  ...
  return (
    <View style={{flex: 1, backgroundColor: 'white'}}>
      ...
      <TextInput
        placeholder="Masukan Jumlah Barang Keluar"
        placeholderTextColor={'#b2b2b2'}
        value={jumlah}
